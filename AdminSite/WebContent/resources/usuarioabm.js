$(document)
		.ready(
				function() {
														...
														data : JSON
																.stringify({
																	...
																	"sexo" : $(
																			'#sexo')
																			.val(),
																	"tipoUsuario" : parseInt($(
																			'#tipoUsuario')
																			.val()),
																	"tiporescatisa" : parseInt($(
																			'#tiposrescatistas')
																			.val()),
																	"residencia" : $(
																			'#direccion')
					...
					var dataFromRes = $('#jsondata').val().toString().replace(
							/'/g, "\"");
					var datares = JSON.parse(dataFromRes);

					for (i = 0; i < datares.length; i++) {
						$('#tiposrescatistas').append(
								'<option value="' + datares[i].value + '">'
										+ datares[i].nombre + '</option>');
					}

					if ($('#idUsu').val() != "") {
						var flag = false;
						var dataFromUsu = $('#jsonusu').val().toString()
								.replace(/'/g, "\"");
						var datarusu = JSON.parse(dataFromUsu);

						if (typeof datarusu.idRescatista != 'undefined') {
							$(
									'#tiposrescatistas option[value="'
											+ datarusu.idTipoRescatista + '"]')
...
function load(keyn, usujson) {
	...
	case "nick":
		...
		break;
	case "sexo":
		$('#sexo option[value="' + usujson[keyn] + '"]').attr("selected",
				"selected");
		break;
	case "tipoUsuario":
		$('#tipoUsuario option[value="' + usujson[keyn] + '"]').attr(
				"selected", "selected");
		break;
	case "nacimiento":
